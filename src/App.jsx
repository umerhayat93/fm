...
function Navbar({ active, setActive, dark, toggleDark }) {
  ...
  return (
    <nav style={{
      ...
      background: scrolled ? (dark ? "rgba(5,5,10,0.92)" : "rgba(255,255,255,0.92)") : "transparent",
        ...
        <div className="desktop-nav" style={{ display: "flex", gap: 28, alignItems: "center" }}>
          {NAV_ITEMS.map(item => (
            <button key={item} onClick={() => scrollTo(item)} style={{
              ...
              color: active === item ? "#c9a84c" : (dark ? "#ccc" : "#333"),
              fontWeight: active === item ? 700 : 400, padding: "4px 0",
              borderBottom: active === item ? "1px solid #c9a84c" : "1px solid transparent",
              transition: "all 0.3s"
            }}>{item}</button>
          ))}
          <button onClick={toggleDark} style={{
            background: "none", border: `1px solid ${dark ? "#c9a84c" : "#999"}`,
            borderRadius: 20, padding: "4px 12px", cursor: "pointer",
            color: dark ? "#c9a84c" : "#555", fontSize: 12
          }}>{dark ? "☀" : "🌙"}</button>
        </div>

        <button onClick={() => setMenuOpen(!menuOpen)} className="mobile-menu-btn" style={{
          display: "none", background: "none", border: "none", cursor: "pointer",
          color: dark ? "#c9a84c" : "#1a1a2e", fontSize: 24
        }}>☰</button>
      </div>

      {menuOpen && (
        <div style={{
          background: dark ? "rgba(5,5,10,0.97)" : "rgba(255,255,255,0.97)",
          padding: "16px 0 24px",
          borderTop: "1px solid rgba(180,145,80,0.2)"
        }}>
          {NAV_ITEMS.map(item => (
            <button key={item} onClick={() => scrollTo(item)} style={{
              ...
              color: dark ? "#ccc" : "#333", padding: "12px 0"
            }}>{item}</button>
          ))}
          <div style={{ textAlign: "center", marginTop: 8 }}>
            <button onClick={toggleDark} style={{
              ...
              color: dark ? "#c9a84c" : "#555", fontSize: 13
...
function Hero({ dark }) {
  ...
  return (
    <section id="home" style={{
      ...
      background: dark
        ? `radial-gradient(ellipse at ${gx}% ${gy}%, #0f0a1e 0%, #05050a 60%, #0a0810 100%)`
        : `radial-gradient(ellipse at ${gx}% ${gy}%, #f0e8d0 0%, #e8e0f0 50%, #d0d8f0 100%)`,
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "100px 5vw 60px", display: "flex", flexWrap: "wrap", alignItems: "center", gap: 60, width: "100%" }}>
        ...
        <div style={{ flex: "1 1 400px", zIndex: 2 }}>
          <div style={{ opacity: loaded ? 1 : 0, transform: loaded ? "translateY(0)" : "translateY(30px)", transition: "all 0.9s cubic-bezier(0.16,1,0.3,1)" }}>
            ...
            <p style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "clamp(15px, 2.2vw, 20px)", color: dark ? "rgba(220,210,190,0.85)" : "rgba(30,20,60,0.75)", lineHeight: 1.6, marginBottom: 12, fontStyle: "italic" }}>
              Overseas Sales Department Project Manager
            </p>
            <p style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "clamp(13px, 1.8vw, 16px)", color: "#c9a84c", letterSpacing: 1, marginBottom: 32 }}>
              PURRPLAY Amusement Equipment Co. Ltd · China
            </p>
            <p style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "clamp(16px, 2.5vw, 22px)", color: dark ? "rgba(240,236,224,0.7)" : "rgba(26,26,46,0.6)", lineHeight: 1.7, marginBottom: 48, fontStyle: "italic" }}>
              "Empowering Sports, Inspiring Youth,<br />Building International Connections."
            </p>
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              {[{ label: "Explore Profile", primary: true }, { label: "Sports Legacy", primary: false }, { label: "Contact", primary: false }].map(btn => (
                <button key={btn.label} style={{
                  ...
                  color: btn.primary ? "#0d0d1a" : (dark ? "#c9a84c" : "#1a1a2e"),
                  border: btn.primary ? "none" : `1px solid ${dark ? "#c9a84c" : "#1a1a2e"}`,
                  fontWeight: btn.primary ? 700 : 500
                }}
                  onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = "0 8px 30px rgba(201,168,76,0.3)"; }}
        ...
        <div style={{ flex: "0 1 340px", display: "flex", justifyContent: "center", opacity: loaded ? 1 : 0, transform: loaded ? "translateY(0)" : "translateY(40px)", transition: "all 1.1s cubic-bezier(0.16,1,0.3,1) 0.2s" }}>
          <div style={{ position: "relative" }}>
            <div style={{
              width: 280, height: 360, borderRadius: 4, overflow: "hidden",
              border: "2px solid rgba(201,168,76,0.35)",
              boxShadow: dark ? "0 40px 80px rgba(0,0,0,0.6), 0 0 60px rgba(201,168,76,0.08)" : "0 40px 80px rgba(0,0,0,0.15), 0 0 60px rgba(201,168,76,0.1)",
              position: "relative"
            }}>
              <img
                src="/fm.jpg"
      ...
      {/* Scroll indicator */}
      <div style={{ position: "absolute", bottom: 32, left: "50%", textAlign: "center", animation: "bounce 2s ease-in-out infinite" }}>
...
function About({ dark }) {
  ...
  return (
    <section id="about" ref={ref} style={{ padding: "120px 5vw", background: dark ? "#05050a" : "#faf8f4" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 80, alignItems: "flex-start" }}>
          ...
          <div style={{ flex: "1 1 400px", opacity: inView ? 1 : 0, transform: inView ? "translateX(0)" : "translateX(40px)", transition: "all 0.9s cubic-bezier(0.16,1,0.3,1) 0.2s" }}>
            ...
            <p style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "clamp(16px, 2.2vw, 20px)", color: dark ? "rgba(220,210,190,0.8)" : "rgba(26,26,46,0.75)", lineHeight: 1.9, marginBottom: 24 }}>
              Having completed his <strong style={{ color: "#c9a84c" }}>PhD</strong>, he combines academic excellence with international business expertise and leadership. Deeply passionate about sports development, youth empowerment, and the promotion of sporting activities.
            </p>
...
function Journey({ dark }) {
  const [ref, inView] = useInView();
  return (
    <section id="journey" ref={ref} style={{ padding: "120px 5vw", background: dark ? "#070710" : "#f4f0ea" }}>
          ...
          {JOURNEY.map((item, i) => (
            <div key={i} style={{ display: "flex", justifyContent: i % 2 === 0 ? "flex-start" : "flex-end", marginBottom: 60, position: "relative", opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(30px)", transition: `all 0.7s cubic-bezier(0.16,1,0.3,1) ${i * 0.12}s` }}>
              <div style={{ width: "44%", padding: "28px 32px", background: dark ? "rgba(255,255,255,0.03)" : "rgba(255,255,255,0.8)", border: `1px solid ${dark ? "rgba(201,168,76,0.2)" : "rgba(201,168,76,0.25)"}`, borderRadius: 4, backdropFilter: "blur(10px)", boxShadow: dark ? "0 20px 60px rgba(0,0,0,0.3)" : "0 20px 60px rgba(0,0,0,0.06)", position: "relative" }}>
                <span style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 32, fontWeight: 900, color: "rgba(201,168,76,0.2)", position: "absolute", top: 16, right: 20 }}>{item.year}</span>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 28 }}>
          {SPORTS.map((s, i) => (
            <div key={i} onMouseEnter={() => setHov(i)} onMouseLeave={() => setHov(null)} style={{
              padding: 36,
              background: hov === i ? (dark ? "rgba(201,168,76,0.08)" : "rgba(201,168,76,0.07)") : (dark ? "rgba(255,255,255,0.02)" : "rgba(255,255,255,0.8)"),
              border: `1px solid ${hov === i ? "rgba(201,168,76,0.5)" : (dark ? "rgba(255,255,255,0.06)" : "rgba(26,26,46,0.08)")}`,
              borderRadius: 4, transition: "all 0.4s ease", cursor: "default",
              transform: hov === i ? "translateY(-6px)" : "translateY(0)",
              boxShadow: hov === i ? "0 24px 60px rgba(201,168,76,0.15)" : "none",
              opacity: inView ? 1 : 0, transitionDelay: `${i * 0.1}s`
            }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 24 }}>
          {LEADERSHIP.map((l, i) => (
            <div key={i} onMouseEnter={() => setHov(i)} onMouseLeave={() => setHov(null)} style={{
              padding: 40,
              background: hov === i ? "linear-gradient(135deg, rgba(201,168,76,0.1), rgba(201,168,76,0.04))" : (dark ? "rgba(255,255,255,0.02)" : "rgba(255,255,255,0.7)"),
              border: `1px solid ${hov === i ? "#c9a84c" : (dark ? "rgba(255,255,255,0.05)" : "rgba(26,26,46,0.08)")}`,
              borderRadius: 4, transition: "all 0.35s ease",
              transform: hov === i ? "translateY(-4px) scale(1.01)" : "translateY(0) scale(1)",
              boxShadow: hov === i ? "0 20px 50px rgba(201,168,76,0.12)" : "none",
              opacity: inView ? 1 : 0, transitionDelay: `${i * 0.08}s`
            }}>
...
function Contact({ dark }) {
  ...
  return (
    <section id="contact" ref={ref} style={{ padding: "120px 5vw", background: dark ? "#070710" : "#f4f0ea" }}>
      <div style={{ maxWidth: 700, margin: "0 auto" }}>
        ...
        {sent ? (
          <div style={{ textAlign: "center", padding: "60px 32px", background: dark ? "rgba(201,168,76,0.06)" : "rgba(201,168,76,0.05)", border: "1px solid rgba(201,168,76,0.3)", borderRadius: 4 }}>
          ...
          <div style={{ opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(30px)", transition: "all 0.8s 0.2s" }}>
            {[{ key: "name", label: "Full Name", type: "text" }, { key: "email", label: "Email Address", type: "email" }].map(f => (
              <div key={f.key} style={{ marginBottom: 24 }}>
                <input
                  ...
                  style={{ width: "100%", background: dark ? "rgba(255,255,255,0.03)" : "rgba(255,255,255,0.8)", border: `1px solid ${dark ? "rgba(201,168,76,0.2)" : "rgba(26,26,46,0.15)"}`, borderRadius: 2, padding: "18px 20px", fontSize: 16, color: dark ? "#f0ece0" : "#0d0d1a", outline: "none", letterSpacing: 0.5, boxSizing: "border-box" }}
                />
              </div>
            ))}
            <div style={{ marginBottom: 28 }}>
              <textarea
                ...
                style={{ width: "100%", background: dark ? "rgba(255,255,255,0.03)" : "rgba(255,255,255,0.8)", border: `1px solid ${dark ? "rgba(201,168,76,0.2)" : "rgba(26,26,46,0.15)"}`, borderRadius: 2, padding: "18px 20px", fontSize: 16, color: dark ? "#f0ece0" : "#0d0d1a", outline: "none", resize: "vertical", boxSizing: "border-box", letterSpacing: 0.5 }}
              />
            </div>
            <button
              onClick={() => setSent(true)}
              style={{ padding: "16px 48px", background: "#c9a84c", color: "#0d0d1a", border: "none", borderRadius: 2, fontSize: 14, letterSpacing: 3, textTransform: "uppercase", fontWeight: 700, cursor: "pointer", transition: "all 0.3s", fontFamily: "'Cormorant Garamond', Georgia, serif" }}
...
function Footer({ dark }) {
  return (
    <footer style={{ padding: "60px 5vw 40px", background: dark ? "#03030a" : "#0d0d1a", textAlign: "center" }}>
      ...
      <p style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 13, color: "rgba(201,168,76,0.3)", letterSpacing: 1 }}>
        © 2025 Fazal Mabood · All Rights Reserved · PURRPLAY Amusement Equipment Co. Ltd
      </p>
    </footer>
  );
}
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: dark ? "#05050a" : "#faf8f4", transition: "background 0.5s ease" }}>
      {/* Scroll progress bar */}
      <div style={{ position: "fixed", top: 0, left: 0, height: 2, width: `${progress}%`, background: "linear-gradient(to right, #c9a84c, #f0d080)", zIndex: 9999, transition: "width 0.1s linear", pointerEvents: "none" }} />
